...
test.describe('Automation Excercise Automation E2E', () => {
  ...
  async function login(page: Page, email: string = validLoginEmail, password: string = loginPassword) {
    await page.fill('input[data-qa="login-email"]', email);
    await page.fill('input[data-qa="login-password"]', password);
    await page.click('button[data-qa="login-button"]');
  }

  async function selectRandomDOB(page: Page, daySelector: string, monthSelector: string, yearSelector: string) {
  ...
  test('Test Case 1: Register User', async ({ page }) => {
    ...
    await expect(page.locator('.text-center').first().locator('b')).toHaveText('Enter Account Information');
    ...
    await expect(page.locator('.text-center').first().locator('b')).toHaveText('Account Created!');
    ...
    const congratulationsMsg = await page.locator('p', { hasText: 'Congratulations! Your new account has been successfully created!' });
    await expect(congratulationsMsg).toBeVisible();

    await page.getByRole('link', { name: 'Continue' }).click();
    await expect(page.getByText(`Logged in as ${personFullName}`)).toBeVisible();
    await expect(page.getByRole('listitem').filter({ hasText: `Logged in as ${personFullName}` })).toBeVisible();

    await page.locator('a', { hasText: 'Delete Account' }).click();

    await expect(page.locator('.text-center').first().locator('b')).toHaveText('Account Deleted!');
    ...
    const deletionsMsg = await page.locator('p', { hasText: 'Your account has been permanently deleted!' });
  ...
  test('Test Case 2: Login User with correct email and password', async ({ page }) => {
    ...
    await expect(page.getByRole('listitem').filter({ hasText: 'Logged in as Yogesh Borole' })).toBeVisible();
  })

  test('Test Case 3: Login User with incorrect email and password', async ({ page }) => {
    await login(page, 'incorrect@gmail.com')

    await expect(page.locator('form').first().locator('p')).toHaveText(/Your email or password is incorrect!/i);
    await expect(page.locator('form').first().locator('p')).toHaveCSS('color', 'rgb(255, 0, 0)');
  })
  ...
  test('Test Case 5: Register User with existing email', async ({ page }) => {
    ...
    await page.locator('form').filter({ hasText: 'Signup' }).getByPlaceholder('Email Address').fill(validLoginEmail)
    await page.getByRole('button', { name: 'Signup' }).click()

    await expect(page.locator('form').locator('p').first()).toHaveText('Email Address already exist!');
    await expect(page.locator('form').locator('p').first()).toHaveCSS('color', 'rgb(255, 0, 0)');
  })
  ...
  test('Test Case 6: Contact Us Form', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Email', exact: true }).fill(randomEmail)
    await page.getByPlaceholder('Subject').fill(faker.lorem.sentence())
    await page.getByRole('textbox', { name: 'Your Message Here' }).fill(faker.lorem.paragraph())
