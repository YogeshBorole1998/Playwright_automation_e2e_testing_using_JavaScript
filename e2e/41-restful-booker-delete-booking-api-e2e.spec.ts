...
test.describe('Restful Booker Delete Booking API Validation', () => {
  ...
  test('@API Delete Booking', async ({ request }) => {
    let ID = bookingId
    const url = `${baseURL}/booking/`
    const response2 = await request.get(url + ID, {})
    expect(response2.status()).toBe(200)

    const deletedResponse = await request.delete(url + ID, {
      headers: {
    ...
    expect(deletedResponse.statusText()).toBe('Created')
